...
export default function SettingsPage() {
  ...
  const { business, credits, userId, refreshBusiness, refreshCredits } = useBusinessContext();
  ...
  }, [activeTab, userId, business?.id]);

  async function fetchTransactions() {
    if (!userId || !business) return;
    setLoadingTx(true);
    try {
      const res = await fetch(`/api/credits?userId=${userId}&businessId=${business.id}`);
      if (res.ok) {
  ...
  async function handleBuyPack(packId: string) {
    ...
    try {
      const res = await fetch("/api/credits/purchase", {
        ...
        body: JSON.stringify({ packId, userId, businessId: business.id }),
      });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
      ...
      {/* Credits */}
      <section className="mb-10" id="credits">
        <h2 className="text-lg font-semibold mb-4" style={{ color: T.text, fontFamily: T.h }}>AI Credits</h2>

        {/* Balance */}
        <div className="p-4 rounded-xl mb-4" style={{ ...glassCard }}>
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-sm" style={{ color: T.text3 }}>Current Balance</p>
              <p className="text-2xl font-bold" style={{ color: T.text, fontFamily: T.h }}>{credits} <span className="text-sm font-normal" style={{ color: T.text3 }}>credits</span></p>
            </div>
            <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: "rgba(123,57,252,0.10)" }}>
          ...
          <p className="text-xs" style={{ color: T.text3 }}>
            Credits are used for AI-powered features like blog generation, ad copy, SEO audits, and more.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          {CREDIT_PACKS.map((pack) => (
            <button
              ...
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = T.purple; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.08)"; }}
            >
              <p className="text-lg font-bold mb-0.5" style={{ color: T.text, fontFamily: T.h }}>{pack.credits}</p>
              <p className="text-xs mb-2" style={{ color: T.text3 }}>credits</p>
              <p className="text-sm font-semibold" style={{ color: T.purpleLight }}>
                ${(pack.priceCents / 100).toFixed(2)}
              </p>
              <p className="text-[10px]" style={{ color: T.text3 }}>
                ${(pack.priceCents / pack.credits / 100 * 100).toFixed(1)}c per credit
              </p>
            </button>
          ))}
        </div>
        ...
        {activeTab === "credits" && (
          <div>
            <h3 className="text-sm font-semibold mb-3" style={{ color: T.text2 }}>Recent Transactions</h3>
            {loadingTx ? (
              <div className="flex items-center justify-center py-8">
                <div className="w-5 h-5 rounded-full animate-spin" style={{ border: `2px solid ${T.purple}`, borderTopColor: "transparent" }} />
              </div>
            ) : transactions.length === 0 ? (
              <p className="text-xs py-4" style={{ color: T.text3 }}>No transactions yet.</p>
            ) : (
              <div className="space-y-1.5">
                {transactions.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between px-3 py-2 rounded-lg" style={{ background: T.glass }}>
                    <div>
                      <p className="text-xs font-medium" style={{ color: T.text }}>
                        {tx.type === "action_spend" ? tx.action?.replace(/_/g, " ") || "AI Action" : tx.type.replace(/_/g, " ")}
                      </p>
                      <p className="text-[10px]" style={{ color: T.text3 }}>
                    ...
                    <div className="text-right">
                      <p className="text-xs font-semibold" style={{ color: tx.amount > 0 ? T.green : "#ef4444" }}>
                        {tx.amount > 0 ? "+" : ""}{tx.amount}
                      </p>
                      <p className="text-[10px]" style={{ color: T.text3 }}>bal: {tx.balance_after}</p>
                    </div>
                  </div>
                ))}
              </div>
