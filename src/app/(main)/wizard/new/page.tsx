...
type Step = "describe" | "persona" | "name" | "building" | "done";

// New business: describe → persona → name
const NEW_STEPS: Step[]      = ["describe", "persona", "name"];
// Existing business: name → describe → persona
const EXISTING_STEPS: Step[] = ["name", "describe", "persona"];

const SERVICE_SUGGESTIONS = [
  { label: "Web design",        fill: "I design and build websites. Been doing it on the side and want to turn it into something real",         subtype: "freelance"   },
  { label: "Consulting",        fill: "I give advice on business and strategy. People pay me to help them figure things out",                   subtype: "consulting"  },
  { label: "Coaching",          fill: "I'm good at helping people work through their goals and get unstuck",                                    subtype: "coaching"    },
  { label: "Marketing",         fill: "I do marketing: content, social, ads. I know how to get attention and drive results",                    subtype: "consulting"  },
  { label: "Bookkeeping",       fill: "I do bookkeeping. I've been handling finances for years and want to go out on my own",                   subtype: "freelance"   },
  { label: "Photography",       fill: "I take photos: portraits, events, brands. Been shooting on the side for years",                         subtype: "freelance"   },
  { label: "Copywriting",       fill: "I write ads, emails, and websites. Good at making words actually work",                                  subtype: "freelance"   },
  { label: "Legal services",    fill: "I have a law background and want to offer consulting and advisory work independently",                   subtype: "consulting"  },
  { label: "Fitness training",  fill: "I train people. Been doing it informally for years and want to do it properly",                         subtype: "coaching"    },
  { label: "IT & tech",         fill: "I fix and set up tech for people. Computers, networks, software. I figure it out",                      subtype: "freelance"   },
  { label: "Interior design",   fill: "I have a good eye for spaces and have been helping friends with their homes",                           subtype: "freelance"   },
  { label: "HR consulting",     fill: "I have an HR background and want to help small companies hire and manage people better",                 subtype: "consulting"  },
];

const PERSONA_OPTIONS = [
  { id: "grinder", exp: "new",         challenge: "clients", label: "Starting fresh",              desc: "Building my first client base, figuring things out" },
  { id: "operator", exp: "building",   challenge: "tools",   label: "I have some clients",         desc: "Actively working with a few, want better systems" },
  { id: "scaler",  exp: "established", challenge: "scale",   label: "I have a solid client base",  desc: "Running steady, want to grow and streamline" },
];
...
function WizardContent() {
  ...
  const [persona, setPersona] = useState<"grinder" | "operator" | "scaler" | null>(null);
      ...
      try {
        ...
        const res = await fetch("/api/generate", {
          ...
          body: JSON.stringify({
            ...
            budget: "",
            bizType: "service",
            hasExistingWebsite: isExisting,
            persona: persona || "operator",
            quizExperience: quizExperience || "building",
            quizChallenge: quizChallenge || "tools",
          }),
        });
    ...
    <div style={{ minHeight: "100vh", background: T.bg }}>
      ...
      <div style={{
        ...
        <AnimatePresence mode="wait">
              ...
              {!isExisting && <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
                {SERVICE_SUGGESTIONS.map((s) => {
                  ...
                  return (
                    <button
                      key={s.label}
                      onClick={() => { setDescription(s.fill); setSelectedSubtype(s.subtype); }}
                      style={{
                        padding: "6px 14px",
                        borderRadius: 100,
                        fontSize: "0.78rem",
          ...
          {/* ── PERSONA ── */}
          {step === "persona" && (
            <motion.div key="persona" {...pt} style={{ maxWidth: 480, width: "100%" }}>
              <button onClick={goBack} style={backBtn}>
                <BackArrow /> Back
              </button>
              <p style={stepLabel}>{stepNum} / {stepTotal}</p>
              <h2 style={headingStyle}>Where are you at right now?</h2>
              <p style={subtitleStyle}>We use this to set up your workspace in the right order for you.</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 28 }}>
                {PERSONA_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      key={opt.id}
                      onClick={() => { setPersona(opt.id as "grinder" | "operator" | "scaler"); setQuizExperience(opt.exp); setQuizChallenge(opt.challenge); }}
                      style={{
                        padding: "14px 16px",
                        borderRadius: 10,
                        background: active ? "rgba(200,164,78,0.08)" : T.bgEl,
                        border: `1px solid ${active ? "rgba(200,164,78,0.35)" : T.border}`,
                        color: T.text,
                        textAlign: "left",
                        cursor: "pointer",
                        transition: "all 0.15s",
                    ...
                    >
                      ...
                      <div style={{ color: T.text3, fontSize: "0.8rem" }}>{opt.desc}</div>
                    </button>
                  );
                })}
              </div>
              <button
                ...
                onClick={() => {
                  const next = QUESTION_STEPS[stepIdx + 1];
                  if (next) { setStep(next); } else { buildStarted.current = false; setStep("building"); }
                }}
                style={primaryBtn(!persona)}
              >
                Continue
              </button>
            </motion.div>
